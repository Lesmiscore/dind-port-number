...
if [ "$#" -eq 0 ] || [ "${1#-}" != "$1" ]; then
	...
	; then
		...
		set -- dockerd \
			--host="$dockerSocket" \
			--host=tcp://0.0.0.0:"${DOCKER_PORT:-2376}" \
			--tlsverify \
			--tlscacert "$DOCKER_TLS_CERTDIR/server/ca.pem" \
			--tlscert "$DOCKER_TLS_CERTDIR/server/cert.pem" \
			--tlskey "$DOCKER_TLS_CERTDIR/server/key.pem" \
			"$@"
		DOCKERD_ROOTLESS_ROOTLESSKIT_FLAGS="${DOCKERD_ROOTLESS_ROOTLESSKIT_FLAGS:-} -p 0.0.0.0:${DOCKER_PORT:-2376}:${DOCKER_PORT:-2376}/tcp"
	else
		# TLS disabled (-e DOCKER_TLS_CERTDIR='') or missing certs
		set -- dockerd \
			--host="$dockerSocket" \
			--host=tcp://0.0.0.0:"${DOCKER_PORT:-2375}" \
			"$@"
		DOCKERD_ROOTLESS_ROOTLESSKIT_FLAGS="${DOCKERD_ROOTLESS_ROOTLESSKIT_FLAGS:-} -p 0.0.0.0:${DOCKER_PORT:-2375}:${DOCKER_PORT:-2375}/tcp"
	fi
fi
